...
function label(type: string) {
  return ({ order_created: "创建订单", order_credited: "确认入账", order_rejected: "拒绝订单", order_canceled: "用户取消" } as Record<string, string>)[type] || type;
}
async function load() {
  loading.value = true;
...
<template>
  <div class="page-stack">
    ...
    <section class="panel table-panel">
      <FilterBar><label class="search-box"><Search :size="18" /><input v-model.trim="tg" inputmode="numeric" placeholder="筛选 Telegram ID" @keyup.enter="load" /></label><label class="select-box"><select v-model="entryType" @change="load"><option value="">全部事件</option><option value="order_created">创建订单</option><option value="order_credited">确认入账</option><option value="order_rejected">拒绝订单</option><option value="order_canceled">用户取消</option></select></label></FilterBar>
      <AdminDataTable :loading="loading" :empty="!items.length" empty-title="暂无账单流水" min-width="900px">
        ...
        <template #body><tr v-for="item in items" :key="item.id"><td>#{{ item.id }}</td><td>TG · {{ item.tg }}</td><td><StatusBadge :label="label(item.entry_type)" :tone="item.entry_type === 'order_credited' ? 'success' : item.entry_type === 'order_rejected' ? 'danger' : 'info'" /></td><td class="strong-cell">{{ money(item.amount_cents) }}</td><td><span class="ledger-coins"><Coins :size="14" /> {{ formatNumber(item.coins) }}</span></td><td>{{ item.description }}</td><td>{{ item.actor_kind }} · {{ item.actor_id }}</td><td>{{ formatDate(item.created_at) }}</td></tr></template>
      </AdminDataTable>
    </section>
  </div>
</template>
